...
import { useGetServiceByIdQuery, useUpdateServiceMutation } from "@/redux/features/admin/service";
...
export default function EditServicePage() {
  ...
  useEffect(() => {
    if (service) {
      ...
      setCategoryId(service.category?.id ? String(service.category.id) : service.category_id ? String(service.category_id) : "NONE");
      setVendorId(service.vendor?.id ? String(service.vendor.id) : service.vendor_id ? String(service.vendor_id) : "NONE");
      setEmployeeIds(service.employees ? service.employees.map((e: any) => Number(e.id)) : []);
      setAgentCommissionPercentage(service.agent_commission_percentage ? String(service.agent_commission_percentage) : "0");
    }
  }, [service]);
      ...
      <div className="flex items-center gap-4 border-b border-slate-100 pb-5">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
            <Sparkles className="text-rose-500" size={20} /> Edit Service
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">Update the details of the service.</p>
        </div>
      </div>
