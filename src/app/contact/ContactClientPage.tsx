          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.24 }} className="flex flex-wrap justify-center gap-2">
            {TRUST_BARS.map(({ icon: Icon, text }) => (
              <span key={text} className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-50 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-full">
                <Icon className="w-3 h-3 text-[#FF6014]" />{text}
              </span>
            ))}
          </motion.div>
        </div>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {CONTACT_CHANNELS.map(({ icon: Icon, label, primary, href, secondary, badge }) => (
                <motion.a key={label} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noopener noreferrer" : undefined} whileHover={{ y: -2 }} transition={{ duration: 0.2 }} className="group relative bg-white border border-slate-100 rounded-2xl p-5 flex items-start gap-4 hover:border-[#FF6014]/25 hover:shadow-[0_4px_24px_rgba(255,96,20,0.08)] transition-all duration-300 overflow-hidden">
                  ...
                  <span className="relative p-2.5 rounded-xl bg-[#FFF4EE] text-[#FF6014] flex-shrink-0 border border-[#FF6014]/10"><Icon className="w-4.5 h-4.5" /></span>
                  <div className="relative min-w-0 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-black text-[9px] uppercase tracking-[.12em] text-slate-400">{label}</h3>
                      <span className="text-[9px] font-bold text-[#FF6014] bg-[#FFF4EE] border border-[#FF6014]/15 px-2 py-0.5 rounded-full whitespace-nowrap">{badge}</span>
                    </div>
                    <p className="text-[13px] font-black text-slate-800 group-hover:text-[#FF6014] transition-colors truncate">{primary}</p>
              ...
              <div className="bg-[#FFF4EE] border border-[#FF6014]/10 rounded-2xl p-5">
                ...
                <div className="flex gap-2">
                  {SOCIAL_LINKS.map(({ icon: Icon, href, label }) => (
                    <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="w-8 h-8 rounded-full bg-white border border-[#FF6014]/15 text-[#FF6014] flex items-center justify-center hover:bg-[#FF6014] hover:text-white hover:border-[#FF6014] transition-all duration-200 hover:scale-110">
          ...
          <RevealSection delay={0.05}>
            <div className="space-y-2">
              {FAQS.map((faq, i) => {
                ...
                return (
                  <div key={i} className={`bg-white border rounded-xl overflow-hidden transition-all duration-200 ${isOpen ? "border-[#FF6014]/20 shadow-[0_2px_12px_rgba(255,96,20,0.06)]" : "border-slate-100"}`}>
                    <button type="button" onClick={() => setActiveFaq(isOpen ? null : i)} className="w-full flex items-center justify-between gap-3 p-4 text-left outline-none">
                      <span className={`text-[12px] font-bold transition-colors ${isOpen ? "text-[#FF6014]" : "text-slate-800"}`}>{faq.question}</span>
                      <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ${isOpen ? "rotate-180 text-[#FF6014]" : "text-slate-300"}`} />
                    </button>
                    <AnimatePresence>
                      {isOpen && (
